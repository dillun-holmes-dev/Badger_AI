...
__all__ = [
    ...
    "NUM_CLASSES", "NUM_BODY_POINTS", "CLASS_MAPPINGS", "CLASS_SKELETONS", "SIGMAS", "FLIP_PAIRS",
...
if os.path.isfile(TRAIN_ANN):
	...
	# Create class mapping dictionary: {internal_label: class_name}
	CLASS_MAPPINGS = {
		CATEGORY_ID_TO_CONTIGUOUS[c["id"]]: c["name"] for c in _cats
	}
	
	# Skeleton connections per class: {internal_label: [[kp_a, kp_b], ...]}
	CLASS_SKELETONS = {
	...
	_first_cat_with_sigmas = next((c for c in _cats if "sigmas" in c), None)
