...
class Compose(object):
    ...
    @staticmethod
    def _apply_transform(transform, image, target, dataset=None):
        if getattr(transform, "requires_dataset", False):
            return transform(image, target, dataset)
    ...
    def default_forward(self, image, target, dataset=None):
        for transform in self.transforms:
            image, target = self._apply_transform(transform, image, target, dataset)
        return image, target
