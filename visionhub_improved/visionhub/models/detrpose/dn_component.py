...
def get_sigmas(num_keypoints, device, sigmas=None):
    sigmas = resolve_sigmas(sigmas, num_keypoints)
...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, num_keypoints, hidden_dim, label_enc, pose_enc, img_dim, device, sigmas=None):
    """
    ...
    if training:
        ...
        center_non_viz = torch.zeros_like(non_viz[:, 0:1], dtype=torch.bool)
        non_viz = torch.cat((center_non_viz, non_viz), dim=1)
        vars = (2 * get_sigmas(num_keypoints, device, sigmas=sigmas)) ** 2
        ...
        positive_idx = torch.arange(len(poses), device=device, dtype=torch.long).unsqueeze(0).repeat(dn_number, 1)
        positive_idx += (torch.arange(dn_number, device=device, dtype=torch.long) * len(poses) * 2).unsqueeze(1)
        positive_idx = positive_idx.flatten()
        negative_idx = positive_idx + len(poses)

        eps = np.finfo('float32').eps
        rand_vector = torch.randn_like(known_poses)
        ...
        padding_label = torch.zeros(
            pad_size,
            hidden_dim * (num_keypoints + 1),
            device=device,
            dtype=input_label_embed.dtype,
        )
        padding_pose = torch.zeros(
            pad_size,
            num_keypoints + 1,
            device=device,
            dtype=input_pose_embed.dtype,
        )
