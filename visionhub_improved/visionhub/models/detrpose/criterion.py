...
from visionhub.misc.box_ops import box_cxcywh_to_xyxy, complete_box_iou_pairwise
...
class Criterion(nn.Module):
    def __init__(self, 
        ...
        self.oks = OKSLoss(linear=True,
                 ...
                 reduction='mean',
                 loss_weight=1.0)
        if hasattr(self.matcher, "set_sigmas"):
            self.matcher.set_sigmas(sigmas)
    ...
    def forward(self, outputs, targets):
        ...
        # Compute the average number of target keypoints across all nodes, for normalization purposes.
        ...
        if "aux_outputs" in outputs:
            ...
            aux_pre = [outputs["aux_pre_outputs"]] if "aux_pre_outputs" in outputs else []
            for i, aux_outputs in enumerate(outputs["aux_outputs"] + aux_pre):
                indices_aux = self.matcher(aux_outputs, targets)
                cached_indices.append(indices_aux)
                indices_aux_list.append(indices_aux)
            for i, aux_outputs in enumerate(outputs.get("aux_interm_outputs", [])):
        ...
        if "dn_aux_outputs" in outputs:
            ...
            for i in range(len(targets)):
                if len(targets[i]['labels']) > 0:
                    t = torch.arange(len(targets[i]['labels']), device=device, dtype=torch.long)
                    t = t.unsqueeze(0).repeat(scalar, 1)
                    tgt_idx = t.flatten()
                    output_idx = (
                        torch.arange(scalar, device=device, dtype=torch.long) * single_pad
                    ).unsqueeze(1) + t
                    output_idx = output_idx.flatten()
                else:
                    output_idx = tgt_idx = torch.empty(0, device=device, dtype=torch.long)
