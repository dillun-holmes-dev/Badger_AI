...
def weighting_function(reg_max, up, reg_scale, deploy=False):
    ...
    project = torch.cat([v.reshape(1) for v in values], 0)
    return project.unsqueeze(0) if deploy else project
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        _lqe_embed = LQE(4, hidden_dim, 2, num_body_points)
    ...
    def forward(self, feats, targets, samples=None):
        ...
        if self.training:
            ...
            if hasattr(self, "anchors") and self.anchors.shape[1] == memory.shape[1]:
        ...
        if self.training and targets is not None and self.dn_number > 0:
            img_dim = samples.shape[-2:] if samples is not None else feats[0].shape[-2:]
